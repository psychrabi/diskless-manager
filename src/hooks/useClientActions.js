...
export const useClientActions = (fetchData, closeContextMenu, setClient, setIsModalOpen) => {
    ...
    const handleAction = useCallback(async (client, action, confirmTitle, confirmDesc, confirmText, invokeCmd, invokeArgs, successMsg, cancelMsg) => {
        if (!client) return;

        // Pre-checks
        if (['reboot', 'shutdown', 'remote'].includes(action) && client.status !== 'Online') {
            showNotification(`Client must be online to ${action}.`, 'error');
            return;
        }
        if (['edit', 'wake', 'reset', 'delete', 'enableSuper', 'disableSuper', 'saveSuper'].includes(action) && client.status === 'Online') {
            ...
            showNotification(`Client must be offline to ${action === 'edit' ? 'make changes' : action}.`, 'error');
            return;
        }
        ...
        try {
            ...
            if (response && response.message) showNotification(response.message, 'success');
    ...
    return {
        ...
        saveSuper: async (client) => {
            ...
            try {
                const response = await invoke('create_snapshot', { token, snapshotName });
