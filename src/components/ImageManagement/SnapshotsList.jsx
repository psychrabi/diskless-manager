...
const SnapshotItem = React.memo(({ snap, masterName, handleDeleteSnapshot, handleRollbackSnapshot }) => (
  <li key={snap.id || snap.name} className="flex flex-wrap justify-between items-center gap-2 p-2 rounded hover:bg-base-100">
    ...
    <div className="flex space-x-1 flex-shrink-0">
      <Button onClick={() => handleRollbackSnapshot(snap.name, masterName)} variant="info" size="icon" className="h-7 w-7" title={`Rollback ${snap.name}`}>
        <RotateCcw className="h-4 w-4" />
      </Button>
      <Button onClick={() => handleDeleteSnapshot(snap.name, masterName)} variant="destructive" size="icon" className="h-7 w-7" title={`Delete ${snap.name}`}>
...
export const SnapshotsList = React.memo(({ master }) => {
