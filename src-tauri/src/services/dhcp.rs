...
impl DhcpService {
    ...
    pub async fn generate_config(&self) -> anyhow::Result<()> {
        ...
        // On Fedora/RHEL the dhcpd daemon runs as the unprivileged `dhcpd`
        // user/group, but the stock `/etc/dhcp` directory is mode-0750 owned by
        // root:root. That would make dhcpd.service exit at runtime with
        // "Can't open /etc/dhcp/dhcpd.conf: Permission denied" even though the
        // config validates. Make the directory and its files readable to the
        // dhcpd group so the daemon can open its configuration.
        ...
        let default_config = match distro {
            ...
            // Arch's dhcpd4.service has no arguments file. Override ExecStart
            // with a systemd drop-in that points at the manager-owned config
            // and appends the serving interfaces to the dhcpd command line.
            crate::platform::Distro::Arch => format!(
                "# Generated by diskless-manager\n[Service]\nExecStart=\nExecStart=/usr/bin/dhcpd -4 -q -cf /etc/dhcp/dhcpd.conf -pf /run/dhcpd4/dhcpd.pid {}\n",
                interfaces
            ),
        };
        write_with_sudo_tee(distro.dhcp_defaults_path(), &default_config).await?;

        info!(
            "DHCP configuration and {} written",
            distro.dhcp_defaults_path()
        );
        Ok(())
    }

    /// Restore the correct SELinux file contexts for the DHCP configuration.
    ///
    /// Files are moved into place from a temp staging directory; depending on
    /// the filesystem layout they can keep a `tmp_t` label afterwards, which
    /// the `dhcpd_t` domain is denied from opening under an enforcing policy.
    /// `sudo dhcpd -t` (no SELinux domain transition) still validates fine,
    ...
    pub async fn start(&self) -> anyhow::Result<()> {
        // Ensure on-disk dhcpd.conf and clients.conf are current and valid
        // before attempting to start the daemon. This self-heals stale
        // configuration (e.g. missing subnet declarations).
    ...
    pub async fn reload(&self) -> anyhow::Result<()> {
        // Validate the on-disk configuration before restarting so a broken
        // dhcpd.conf does not leave the service dead.
        self.generate_config().await?;
        let service = crate::platform::detect().dhcp_service();
    ...
    pub async fn get_config(&self) -> anyhow::Result<String> {
        ...
        if let Some(content) = crate::infrastructure::command::read_file_with_sudo(&config_path)? {
            Ok(content)
        } else {
            validate_dhcp_config(&self.settings.dhcp)?;
            validate_server_consistency(&self.settings)?;
    ...
    async fn render_client_configs(&self) -> anyhow::Result<String> {
        ...
        let client_config = clients
            ...
            .map(|client| {
                ...
                {
                    // Provisioned clients receive the iSCSI root path so the
                    // DHCP server points them at their boot target.
                    Some(iqn) => crate::infrastructure::dhcp::create_dhcp_entry_for_server(
                        &client.name,
                        &client.mac,
                        &client.ip,
                        iqn,
                    ...
                    // Registered-but-unassigned (enrolled) clients still get a
                    // pinned MAC/IP block; without a root path they fall back to
                    // the subnet's autoexec.ipxe dispatch loop.
                    None => crate::infrastructure::dhcp::create_pending_dhcp_entry(
                        &client.name,
                        &client.mac,
                        &client.ip,
                    ),
