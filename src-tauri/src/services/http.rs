...
use crate::infrastructure::{
    ...
    pxe::render_client_dispatcher_autoexec_with_scheme,
};
use crate::services::{
...
impl HttpService {
    pub async fn generate_config(&self) -> anyhow::Result<()> {
        ...
        if self.settings.http.tls_enabled {
            for path in [
                &self.settings.http.tls_cert_path,
                &self.settings.http.tls_key_path,
            ] {
        ...
        let tls_config = if self.settings.http.tls_enabled {
            format!(
                "    SSLEngine on\n    SSLCertificateFile {}\n    SSLCertificateKeyFile {}\n",
                self.settings.http.tls_cert_path, self.settings.http.tls_key_path
            )
        } else {
            String::new()
        };
...
"#,
        ...
        if distro.is_debian() {
            ...
            if self.settings.http.tls_enabled {
                run_sudo_command(["a2enmod", "ssl"]).await?;
            }
    ...
    async fn publish_autoexec(&self) -> anyhow::Result<()> {
        let autoexec = render_client_dispatcher_autoexec_with_scheme(
            self.settings.http.port,
            crate::infrastructure::pxe::ENROLL_PORT,
            self.settings.http.tls_enabled,
        );
        let root_dir = PathBuf::from(&self.settings.http.root_dir);
    ...
    async fn publish_cached_client_menus(&self) -> anyhow::Result<()> {
        ...
        for client in config.clients {
            ...
            let chap = match (
                ...
                (Some(true), Some(username), Some(password))
