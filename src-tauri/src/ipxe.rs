...
///
/// The DHCP `root-path` identifies the client's persistent iSCSI disk. We
/// attach that disk first, then attempt a normal SAN boot. If the disk is
/// empty/uninstalled (or its Windows bootloader is not usable), `sanboot`
/// returns to the script and the same attached disk is retained while WinPE
/// is loaded. This is the first-boot Windows installation path.
#[must_use]
pub fn render_autoexec_ipxe() -> &'static str {
    r#"#!ipxe
# Generated by diskless-manager.
# Normal Windows SAN boot with automatic WinPE provisioning fallback.
...
echo The iSCSI target may be unavailable or the WinPE files may be missing.
shell
