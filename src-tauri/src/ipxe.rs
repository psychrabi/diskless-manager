...
///
/// DHCP supplies a client-specific iSCSI `root-path`, so this script must not
/// contain a target IQN or any client-specific storage identifier.
#[must_use]
...
mod tests {
    ...
    #[test]
    fn autoexec_is_generic_and_uses_dhcp_root_path() {
        let script = render_autoexec_ipxe();
        assert!(script.starts_with("#!ipxe"));
        assert!(script.contains("dhcp"));
        assert!(script.contains("isset ${root-path}"));
