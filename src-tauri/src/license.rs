...
pub fn activate_license(key: &str) -> Result<String, AuthError> {
    // verify with remote
    let res = verify_license_remote(key).map_err(|e| AuthError { message: e })?;
    if !res.valid {
    ...
    settings.insert(
        ...
        serde_json::to_value(key).map_err(|e| AuthError { message: format!("failed to serialize license key: {}", e) })?,
    );
    settings.insert(
        "license_status".to_string(),
        serde_json::to_value("valid").map_err(|e| AuthError { message: format!("failed to serialize license status: {}", e) })?,
    );
    if let Some(expires) = res.expires_at {
        settings.insert(
            "license_expires".to_string(),
            serde_json::to_value(expires).map_err(|e| AuthError { message: format!("failed to serialize expires: {}", e) })?,
        );
    }
    cfg.settings = serde_json::Value::Object(settings);
    config::write_config(&cfg).map_err(|e| AuthError { message: format!("failed to save license: {}", e) })?;
...
pub fn ensure_license_valid() -> Result<(), AuthError> {
    ...
    Err(AuthError { message: "License not activated. Please activate to use the application.".to_string() })
}
